...
const panierData = (param_copy, param_produit, param_body) => {
    ...
    listenChange();
    getId('totalPanier').textContent=totalPanier(); //ok nécessaire lors du reload
}
...
const sendingCommand=()=>{
    getId('toConfirmation').addEventListener('click', (e) => {
...
//stockage des données renvoyée par le serveur dans le localStorage
...
const filling = (param_customer, param_order, param_total) => {
    getId('customerName').textContent = getItem(param_customer).lastName;
    getId('commandNumber').textContent = getItem(param_order);
    getId('totalAmount').textContent = getItem(param_total);
}
